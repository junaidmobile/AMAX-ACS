...
import { RecordDeliveryDetail } from './Record-Delivery-Detail';
...
@Component({
  selector: 'Record-Delivery',
  templateUrl: 'Record-Delivery.html'
})
export class RecordDelivery implements OnInit {
  scanData: {};
  ...
  getGPList() {
    ...
    this.http.getHttpPostRequest(Constants.GMAX_Services.Imports.GetHAWBNosBasedOnMAWBNos_HHT_MAWB, this._allPrams).then((response) => {
      if (response != null && response != "") {
        ...
        // for (var i = 0; i < this.dropdownHAWB.length; i++) {
        //   if ( this.dropdownHAWB[i]['HAWBNo'] == "") {
        ...
        //       this.global.showAlert("Delivery cannot be recorded. Shipment delivery is not accepted by the Customs Broker");
        //       return;
        //     }

        //     this.clearInputs();
        //     this.global.routePage(RecordDeliveryDetail);
        //   } else {
        //     // if (this.GPDeliveryStatus == "") {
        //     //   this.global.showAlert("Delivery cannot be recorded. Shipment delivery is not accepted by the Customs Broker");
        //     //   return;
  ...
  ionViewWillLeave() {
    this.fabGone = true;
    localStorage.removeItem('GPNumber'); // new for OTP issue added on 29/04
  }
  focusNextInput() {
      ...
      if (response != null && response != "") {
        ...
        if (response.hasOwnProperty('NewDataSet')) {
          ...
          for (var j = 0; j < this.GPdetailsNew.length; j++) {
            // if (this.GPdetailsNew[j]['GPDeliveryStatus'] == 'Verified') {
              this.GPdetails = [];
