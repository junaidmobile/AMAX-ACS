...
export class ExportVehicleCSC { pi_ArrAWBNo: any; isDesktop: boolean; pi_strUserName: any }
export class VehicleModelCSC { LaneMarkingArea: any; GateIn: any; DockIn: any; DockOut: any; GateOut: any }
@Component({
  selector: 'page-export-Vehicle-tracking-csc',
  templateUrl: 'Vehicle-tracking.html'
})

export class VehicleTrackingCSC implements OnInit {
  exportVehicle: ExportVehicleCSC
  Prefix: any;
  MAWBNo: any;
  ...
  constructor(public navCtrl: NavController, public alertCtrl: AlertController, public http: HttpProvider, public global: GlobalProvider) {
    this.exportVehicle = new ExportVehicleCSC();
    this.vehicleModel = new VehicleModelCSC();
  ...
  GetVehicleDetails() {
    ...
    debugger;
    //  this.exportVehicle.pi_ArrAWBNo = '<AWBInfo><GeneralDet AWBNo="' + this.Prefix + this.MAWBNo + '"/></AWBInfo>';
  ...
  fetchVehicleDetails() {
    ...
    this.http.getHttpPostRequest(Constants.GMAX_Services.Exports.Vehicle_tracking, this.exportVehicle, true).then((response) => {
  ...
  getVehicleDetails(url, service_name) {
    this.http.getHttpPostRequest1(url, service_name, this.exportVehicle, true).then((response) => {
      //console.log("Response : ", response);
      if (response != null && response != "") {
        this.setVehicleDetails(response)
      } else {
